...
class Intercomm extends EventEmitter {
  ...
  exec(at, method, parameters) {
    ...
    var requestData = this.messageAPI.request.rpc(method, parameters)
  ...
  handleResponseMessage(response) {
    ...
    // TODO - There is a fundamental design error here
    // The response's id should be independent of the request's id.
    // The correct way of implementing would be adding a `requestId`
    // property to the response object.
    let request = this.requestManager.getRequest(response.id, { error: false })
    if (!request) {
  ...
  handleRequestMessage(request) {
    ...
    if (typeof fn !== 'function') {
      ...
      process.nextTick(() => {
        ...
        let response = this.messageAPI.response.error(request)
      ...
      .catch(err => {
        var response = this.messageAPI.response.error(request)
