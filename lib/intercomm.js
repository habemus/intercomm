...
Intercomm.prototype.exec = function (at, method, parameters) {
  ...
  try {
    ...
    var defer = _defer(self.requestTimeout);

    // json-message assures the defer property won't be stringified
    request.defer = defer;

    self._sentRequests[request.id] = request;
    ...
    // send the message and then return the defer's promise
    return Bluebird.resolve(self.sendMessage(request))
    .catch(function (err) {
      // only catch sendMessage's errors
      // other errors should be considered operational
      // and out of intercomm's scope
      return Bluebird.reject(new errors.SendMessageError(err));
    })
    .then(function () {
      // return the promise
