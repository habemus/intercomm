...
class Intercomm extends EventEmitter {
  constructor(options) {
     ...
     * @optional
     */
    this.prepareResponseMessage = options.prepareResponseMessage || this.prepareResponseMessage
     ...
     * @optional
     */
    this.prepareEventMessage = options.prepareEventMessage || this.prepareEventMessage
     ...
     * @optional
     */
    this.prepareErrorResponseMessage = options.prepareErrorResponseMessage || this.prepareErrorResponseMessage
    ...
    if (this.id === '*') { throw new Error('`*` is a reserved expression, thus not a valid id') }
   ...
   * @param  {String | Array[String]} at id(s) of node(s) at which the event should be emitted
   * @param  {String} eventName 
   * @param  {Object} data
   */
  emitRemote(at, eventName, data) {
    ...
    if (at === '*') {
      ...
      let eventMsg = this.messageAPI.event(eventName)
      this.prepareEventMessage(eventMsg, data)
      eventMsg.from = this.id

      this.sendMessage(eventMsg)
    } else {
      // has specific destinations
  ...
  handleRequestMessage(request) {
    ...
    if (typeof fn !== 'function') {
      ...
      process.nextTick(() => {
        ...
        this.prepareErrorResponseMessage(request, response, error)
      ...
      return promiseTry(fn, request.params).then(result => {
        ...
        // call user defined `prepareResponseMessage` method
        // to load the result into the response object
        this.prepareResponseMessage(request, response, result)
      ...
      .catch(err => {
        ...
        this.prepareErrorResponseMessage(request, response, err)
   ...
   * @param  {Error}
   */
  prepareErrorResponseMessage(request, response, err) {
    response.load({
      name: err.name,
      message: err.message,
    }, DEFAULT_ERROR_DATA)
   ...
   * @param  {*} data
   */
  prepareResponseMessage(request, response, data) {
    if (Array.isArray(data)) {
      data = data.map(item => toJSON(item))
    } else {
      data = toJSON(data)
    }
   ...
   * @param  {*} data
   */
  prepareEventMessage(eventMessage, data) {
  ...
  handleEventMessage(eventMsg) {
    if (eventMsg.from) {
      this.emit(`${eventMsg.from}:${eventMsg.eventName}`, eventMsg.data)
    }
